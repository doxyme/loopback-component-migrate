  ...
  if (!options.enableRest) {
    if (Migration.disableRemoteMethodByName) {
      ...
      Migration.sharedClass.methods().forEach(function(method) {
        Migration.disableRemoteMethodByName(method.name);
      });
      MigrationMap.sharedClass.methods().forEach(function(method) {
        MigrationMap.disableRemoteMethodByName(method.name);
      });
    } else {
      // Loopback 2.x
      ...
      Migration.sharedClass.methods().forEach(function(method) {
        Migration.disableRemoteMethod(method.name, method.isStatic);
      });
      MigrationMap.sharedClass.methods().forEach(function(method) {
        MigrationMap.disableRemoteMethod(method.name, method.isStatic);
      });
    }
  }
};
